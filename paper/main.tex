...
Added certificate-noise and certificate-corruption stresses. The paper is now
explicit that the mechanism depends on conservative but not overbroad robot
...
A v2 corruption stress shows the boundary: 50\% certificate false negatives reduce
self-occluded keep rate from 0.920 to 0.674, while 50\% false positives raise stale
clear-absence rate from 0.143 to 0.544.
\end{abstract}
...
\begin{table}[t]
\caption{V2 certificate-corruption stress at 42 degree self-occlusion. False negatives make real self-occlusions count as clear misses; false positives make clear misses look robot-occluded.}
\label{tab:corruption}
\centering
\small
\input{../experiments/certificate_corruption_table.tex}
\end{table}

Table~\ref{tab:corruption} makes the calibration boundary explicit. False-negative
certificates reduce the very persistence benefit the method is meant to provide:
at 50\% false negatives, keep-under-self-occlusion falls to 0.674. False positives
recover the failure mode of long memory: at 50\% false positives, stale clear-absence
rate rises to 0.544. The method therefore needs conservative visibility certificates,
but certificates that are too broad can preserve objects that are actually gone.
...
The v2 corruption stress quantifies this limitation rather than treating it as a footnote.
